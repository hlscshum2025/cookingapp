...
export type PantryDraft={ingredientKey:string;name:string;category?:string};
...
export async function loadPantryItems():Promise<PantryItem[]>{
  ...
  const {data,error}=await s.from("pantry_items")
    .select("id,ingredient_key,name,category,created_at,updated_at")
    .eq("owner_id",user.id)
...
export async function addPantryItemByName(name:string){
  return savePantryItem(pantryDraftFromName(name));
}
