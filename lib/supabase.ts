...
export async function loadCloudData(userId:string){
  ...
  return {recipes:(recipes.data||[]).map(r=>r.document as Recipe),logs:(logs.data||[]).map(r=>r.document as CookingLog),ingredients:(ingredients.data||[]).map(r=>r.document as IngredientMapping)};
}
...
export async function persistLog(log:CookingLog){const s=getSupabase();if(!s)return;const {data:{user}}=await s.auth.getUser();if(!user)return;const {error}=await s.from("cooking_logs").upsert({id:log.id,owner_id:user.id,recipe_id:log.recipeId,cooked_at:log.cookedAt,rating:log.rating,document:log});if(error)throw error;}
