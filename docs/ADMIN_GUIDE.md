...
6. 第一次选择“先试导入前 10 条”，确认新增 10、失败 0。
7. 再选同一文件、仍导入前 10 条，确认新增 0、重复 10。
8. 在 `import_jobs` 查看两次任务，在 `import_items` 查看逐条状态，在 `source_videos` 查看原始元数据。
9. 前 10 条核验完成后，再选择“导入全部 199 条”；前 10 条会自动计为重复。
10. 打开菜谱库，确认新增内容均为私密“待整理”，再逐条补充可靠内容。
...
- Free 项目如果过去 7 天数据库活动过低，可能被暂停。Supabase 当前说明是“一周内每天有少量用户数据库请求通常足够”；正常打开 CookingApp、登录、查看或编辑菜谱都会产生请求；
- 只登录 Supabase 账号但不进入项目，不应当当作可靠保活方式。收到预警邮件后，进入具体项目 Dashboard，或打开 CookingApp 做几次真实查询即可产生项目活动；
- 不需要每天手工登录。正式使用网页后，日常访问一般就足够；如果连续一周不用，暂停后可在 Dashboard 点击 `Resume project` 恢复；
- 免费方案不应被当作唯一备份；
- 设置页导出的 JSON 会读取你在 10 张业务表中的全部记录，包括导入审计；它不包含 Storage 中的图片二进制文件；
- 当前 199 条 B 站元数据原文件仅约 139 KiB。即使加入结构化菜谱和审计，早期 JSON 通常仍只是几 MB，普通笔记本下载只需很短时间；
- 正式依赖该网站后，应制定数据库和图片的独立备份方案。

官方说明：<https://supabase.com/docs/guides/platform/free-project-pausing>
