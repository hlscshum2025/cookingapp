...
2026-08-26 用户已确认上一轮“当前基线与本轮验收”全部通过；`V2-UX-01`、`V2-RECIPE-01`、`V2-FIN-01`、`V2-MOBILE-01～03` 已移入 `00-V2`，不再作为开放任务重复出现。

| ID | 当前任务 | 状态 | 用户需要检查什么 |
|---|---|---|---|
| V2-FIN-02 | 餐食财务无刷新切换与聚餐角色认领 | 代码完成，待本地验收 | 三个功能在同一工作区点击切换，无需刷新；菜品可展开/收回；帮厨、采购可认领并通过红色“我有点忙”取消认领。 |
...
| ID | 当前任务 | 状态 | 完成定义 |
|---|---|---|---|
| V2-OCR-01 | 统一 `ReceiptOcrDraft` 契约 | Python 契约与 JSON 输出已建立，待真实样本校准 | 文本行、bbox、置信度、商品/数量/单位/价格候选和 provider/version 结构固定。 |
| V2-OCR-02 | OpenCV + PaddleOCR baseline | 可运行框架已建立，待安装模型与真票实验 | 完成灰度、去噪、阈值、轮廓、透视矫正与 OCR；记录字符、行分组和商品行错误。 |
...
| V2-VISION-02 | YOLO / YOLO-World baseline | 推理入口已建立，待真实图片实验 | 对苹果、香蕉、番茄、鸡蛋、牛奶等真实照片记录 precision/recall、误检与漏检。 |
...
1. 本地验收餐食财务无刷新切换、菜品展开/收回及帮厨/采购认领。
2. 在 PyCharm 安装 `vision` 环境，用真实德国小票跑 OpenCV/PaddleOCR baseline 并记录错误。
3. 用冰箱、台面和袋装食材照片跑 YOLO-World baseline，再决定微调、数据量和硬件接口。
4. 完成小红书截图 OCR，并确保候选逐项确认。
