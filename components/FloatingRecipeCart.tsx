...
export function FloatingRecipeCart(){
  const pathname=usePathname();const {items,remove,clear}=useRecipeCart();const [open,setOpen]=useState(false);
  ...
  return <div className="recipe-cart-float">
    {open&&<section className="recipe-cart-popover" aria-label="已选菜谱">
      ...
      <Link href="/translations" className="btn btn-primary" style={{width:"100%",marginTop:14,opacity:items.length?1:.55,pointerEvents:items.length?"auto":"none"}} onClick={()=>setOpen(false)}>生成采购清单 →</Link>
    </section>}
