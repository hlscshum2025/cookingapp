...
export function UniversalSourceImport(){
  ...
  const copyXhsExtractor=async()=>{
    ...
    }catch(reason){setError(reason instanceof Error?reason.message:"复制网页提取脚本失败。");}
  ...
  const detect=async()=>{
    ...
    if(initial.platform==="xiaohongshu"){
      if(initial.browserExtracted){
        setMessage(initial.extractedRecipe?`已读取小红书浏览器提取 JSON：${initial.extractedRecipe.ingredients.length} 条食材、${initial.extractedRecipe.steps.length} 个步骤。保存后会预填手工录入。`:"已读取小红书浏览器提取 JSON。正文会保存到来源记录；当前没有自动拆出完整食材/步骤，可以进入手工录入继续整理。");
      }else{
        setMessage("小红书分享链接已识别。由于平台会拦截服务器自动读取，请按下方“小红书网页提取器”说明，在你自己的浏览器里完成验证后提取正文。也可以只保存这条来源链接。 ");
      }
      return;
    }
  ...
  return <section className="panel" style={{marginBottom:18}}>
    <div className="section-head" style={{marginTop:0,alignItems:"flex-start"}}>
      <div><p className="eyebrow">SHARE / URL IMPORT</p><h2>链接与网页提取导入</h2><p className="subtitle">下厨房直接读公开 MIP 页面；小红书由你的浏览器先通过登录/验证，再把已经显示出来的正文带回 CookingApp。</p></div><span className="badge">单条来源</span>
    </div>
    ...
    <div className="two-col" style={{marginBottom:18}}>
      ...
      <div className="notice" style={{background:"#fff4e8"}}><b>小红书：浏览器网页提取器</b><br/>服务器自动抓取容易被小红书拦截，所以改成：你先打开笔记并自己完成登录/验证 → 运行提取脚本 → 复制 CookingApp JSON → 回这里粘贴。脚本只读取当前页面已经显示给你的内容，不读取 Cookie 或账号令牌。</div>
    </div>

    <div className="field"><label>粘贴下厨房链接、小红书分享文本，或“小红书网页提取 JSON”</label><textarea value={input} onChange={event=>{setInput(event.target.value);setDraft(null);setError("");setMessage("");}} placeholder={"下厨房：\nhttps://www.xiachufang.com/recipe/123456/\n\n小红书：先粘贴分享文本定位来源；需要自动读取正文时，再粘贴网页提取器生成的 JSON。"} style={{minHeight:140}}/><small>这里不会要求你提供 Cookie、SESSDATA、xsec_token 以外的账号凭据，也不会尝试绕过滑块验证码或平台登录。</small></div>

    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:12}}>
      <button type="button" className="btn btn-secondary" onClick={()=>void detect()} disabled={!input.trim()||reading}>{reading?"正在读取下厨房 MIP…":"识别来源"}</button>
      <button type="button" className="btn btn-secondary" onClick={()=>void copyXhsExtractor()} disabled={copyingExtractor}>{copyingExtractor?"正在复制…":"复制小红书网页提取脚本"}</button>
    </div>

    <details style={{marginTop:14}}><summary style={{cursor:"pointer",fontWeight:800}}>小红书网页提取器怎么用？</summary><div className="notice" style={{marginTop:10}}><ol style={{margin:"0 0 0 20px",padding:0,display:"grid",gap:8}}><li>点上面的“复制小红书网页提取脚本”。</li><li>打开目标小红书笔记；如果先出现失败、重新加载、登录或人机验证，由你正常完成。</li><li>笔记正文已经能看到后，按 F12 → Console / 控制台，把刚才脚本粘贴进去并回车。</li><li>页面右上角会出现“CookingApp 小红书提取器”，点“复制 CookingApp JSON”。</li><li>回到这个输入框粘贴 JSON → 点“识别来源” → 保存。食材和步骤能识别出来时会直接预填。</li></ol><p className="subtitle" style={{marginBottom:0}}>如果浏览器不允许直接粘贴 Console，可以先在控制台手动输入允许粘贴提示后再粘；不要把 Cookie、密码或开发者工具里的请求令牌复制给 CookingApp。</p></div></details>

    {draft&&<div className="notice" style={{marginTop:16,background:"var(--leaf-soft)"}}>
      <div className="section-head" style={{marginTop:0}}><div><b>识别为：{draft.platformLabel}</b><p className="subtitle" style={{margin:"4px 0 0"}}>{draft.platform==="xiachufang"?"下厨房会优先使用 MIP 自动读取。":draft.platform==="xiaohongshu"?(draft.browserExtracted?"这是浏览器网页提取结果。":"当前只是分享链接；如需正文自动预填，请使用网页提取器。 "):"保存后进入待处理来源。"}</p></div><span className="badge">{draft.externalId}</span></div>
      <div className="form-grid" style={{marginTop:14}}>
        ...
        <div className="field full"><label>来源链接</label><a className="btn btn-secondary" href={draft.url} target="_blank" rel="noreferrer" style={{justifyContent:"flex-start",overflowWrap:"anywhere",height:"auto",minHeight:42}}>打开原页面 ↗</a><small style={{overflowWrap:"anywhere"}}>{draft.url}</small></div>
        <div className="field full"><label>读取到的正文／备注</label><textarea value={draft.description} onChange={event=>update("description",event.target.value)} placeholder="没有读取到正文时，可保留分享文字作为参考"/></div>
      </div>
      {draft.extractedRecipe&&<div className="notice" style={{marginTop:12,background:"rgba(255,255,255,.55)"}}><b>结构化内容已就绪</b><br/>食材 {draft.extractedRecipe.ingredients.length} 条 · 步骤 {draft.extractedRecipe.steps.length} 个。保存来源后进入手动录入会自动填入，但仍保持“未核验”。</div>}
