...
export function UniversalSourceImport(){
  ...
  const detect=async()=>{
    ...
    if(initial.platform==="xiaohongshu"){
      ...
      else setMessage("小红书分享链接已识别。由于平台会拦截服务器自动读取，请按下方“小红书网页提取器”说明，在你自己的浏览器里完成验证后提取正文。也可以只保存这条来源链接。 ");
      return;
    }
  ...
  return <section className="panel" style={{marginBottom:18}}>
    ...
    <div className="two-col" style={{marginBottom:18}}>
      ...
      <div className="notice" style={{background:"#fff4e8"}}><b>小红书：浏览器网页提取器</b><br/>服务器自动抓取容易被小红书拦截，所以改成：你先打开笔记并自己完成登录/验证 → 运行提取脚本 → 复制 CookingApp JSON → 回这里粘贴。脚本只读取当前页面已经显示给你的内容，不读取 Cookie、密码或账号令牌。</div>
    </div>
