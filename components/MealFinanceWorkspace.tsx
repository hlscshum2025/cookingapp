...
type Dish={id:string;name:string;chef:string;votes:number;claims:Record<RoleKey,boolean>};

const dishSeed:Dish[]=[
  {id:"braised-pork",name:"红烧肉",chef:"越",votes:5,claims:{helper:false,buyer:false}},
  {id:"salad",name:"凉拌黄瓜",chef:"林",votes:3,claims:{helper:false,buyer:false}},
  {id:"rice",name:"米饭与饮料",chef:"安",votes:4,claims:{helper:false,buyer:false}},
];
...
const pageMeta:Record<MealFinanceModule,{eyebrow:string;title:string;subtitle:string;badge:string;warn?:boolean}>={
  ...
  ledger:{eyebrow:"FOOD LEDGER",title:"饮食记账",subtitle:"上传小票后只提取食品支出；先核对商品与分类，再写入个人账本和采购历史。",badge:"GUI 草图 · OCR 未接入",warn:true},
};
...
export function MealFinanceWorkspace({initialActive}:{initialActive:MealFinanceModule}){
  ...
  const [dishes,setDishes]=useState(dishSeed);
  ...
  return <div className="page">
    ...
    <div className="meal-finance-view" aria-live="polite">
      ...
      {active==="gatherings"&&<GatheringsView dishes={dishes} openDishId={openDishId} note={note} setNote={setNote} setOpenDishId={setOpenDishId} toggleVote={toggleVote} toggleClaim={toggleClaim}/>}
...
function GatheringsView({dishes,openDishId,note,setNote,setOpenDishId,toggleVote,toggleClaim}:{dishes:Dish[];openDishId:string|null;note:string;setNote:(value:string)=>void;setOpenDishId:(value:string|null)=>void;toggleVote:(id:string)=>void;toggleClaim:(id:string,role:RoleKey)=>void}){
  ...
  return <><section className="gathering-hero panel"><div><span className="gathering-date">周六 · 18:30</span><h2>夏末家常菜聚餐</h2><p className="subtitle">6 人 · 越家厨房 · 距离锁定菜单还有 2 天</p></div><div><small className="demo-label">演示成员</small><div className="gathering-people" aria-label="演示参与成员"><span>越</span><span>林</span><span>安</span><span>＋3</span></div></div><button className="btn btn-primary" disabled>邀请成员（后续）</button></section>
    <div className="finance-layout"><section className="panel"><div className="section-head"><div><p className="eyebrow">MENU & ROLES</p><h2>点菜与分工</h2><p className="subtitle">点击一道已经挂上去的菜展开任务，再点一次即可收回。</p></div><button className="btn btn-secondary" disabled>＋ 挂一道菜</button></div><div className="dish-task-list">{dishes.map(item=>{const open=openDishId===item.id;return <article key={item.id} className={open?"is-open":""}><div className="dish-summary"><button className="dish-vote" onClick={()=>toggleVote(item.id)} aria-label={`支持 ${item.name}`}><b>♡</b><span>{item.votes}</span></button><button className="dish-expand" type="button" aria-expanded={open} onClick={()=>setOpenDishId(open?null:item.id)}><span><h3>{item.name}</h3><small>主厨：{item.chef}</small></span><b aria-hidden="true">{open?"收起 −":"分工 ＋"}</b></button></div>{open&&<div className="dish-role-list">{roles.map(([role,label,description])=>{const claimed=item.claims[role];return <div className="dish-role" key={role}><div><b>{label}</b><small>{description}</small></div><span className={`task-status ${claimed?"":"open"}`}>{claimed?"已认领 · 我":"待认领"}</span><button type="button" className={`btn ${claimed?"btn-danger claim-cancel":"btn-secondary"}`} onClick={()=>toggleClaim(item.id,role)}>{claimed?"我有点忙":"我来帮忙"}</button></div>})}</div>}</article>})}</div><div className="field finance-note"><label>给聚餐成员的说明</label><textarea value={note} onChange={event=>setNote(event.target.value)} placeholder="例如：有人对坚果过敏；采购时保留小票"/></div></section>
...
function LedgerView({fileName,items,foodTotal,setFileName,toggleItem}:{fileName:string;items:typeof ledgerSeed;foodTotal:number;setFileName:(value:string)=>void;toggleItem:(id:string)=>void}){
  return <><div className="finance-layout"><section className="panel"><div className="section-head"><div><p className="eyebrow">RECEIPT INBOX</p><h2>小票收件箱</h2></div><span className="badge">1 待核对</span></div><label className="receipt-drop"><input type="file" accept="image/*,.pdf" onChange={event=>setFileName(event.target.files?.[0]?.name||"")}/><b>▣</b><span><strong>{fileName||"选择小票照片或 PDF"}</strong><small>{fileName?"文件只在当前界面暂存，尚未上传。":"支持相机照片、截图和扫描件；正式版支持批量。"}</small></span><em>{fileName?"已选择":"浏览文件"}</em></label><div className="receipt-steps"><span className="done">1 上传</span><span className={fileName?"current":""}>2 识别</span><span>3 核对</span><span>4 记账</span></div><button className="btn btn-primary full-width-action" disabled>识别并建立草稿（下一步接 OCR）</button></section>
