...
export function RecipeCard({recipe,href,coverUrl,publicCard=false,likeCount=0,liked=false,likeBusy=false,onToggleLike}:RecipeCardProps) {
  ...
  return <div className="recipe-card-shell">
    <Link href={href||`/recipes/${recipe.id}`} className="recipe-card"><div className={`recipe-cover ${image?"has-image":""}`} style={coverStyle}>{!image&&<span className="cover-emoji">{recipe.emoji}</span>}<span className="cover-status">{statusLabel[recipe.status]}</span></div><div className="recipe-body"><h3 className="recipe-title" title={recipe.title}>{recipe.title}</h3><p className="recipe-desc" title={recipe.summary}>{recipe.summary}</p><div className="meta-row"><span>◷ {recipe.totalMinutes||"?"} 分钟</span><span>♙ {recipe.servings} 人份</span><span>◇ {recipe.difficulty}</span>{recipe.riskLevel&&<span className={`mini-risk risk-${recipe.riskLevel}`}>◉ {recipe.riskLevel}</span>}{recipe.effortType&&<span>⌛ {recipe.effortType}</span>}</div><div className="tag-row">{recipe.tags.slice(0,3).map(tag=><span className="tag" key={tag}>{tag}</span>)}</div></div></Link>
