...
export function RecipeCard({recipe,href,coverUrl,publicCard=false,likeCount=0,liked=false,likeBusy=false,onToggleLike}:RecipeCardProps) {
  ...
  const coverStyle=image
    ?{backgroundImage:`linear-gradient(180deg,rgba(24,31,27,.04),rgba(24,31,27,.28)),url(${JSON.stringify(image)})`,backgroundSize:"cover",backgroundPosition:"center"}
    :{background:recipe.color};
  return <div className="recipe-card-shell">
    <Link href={href||`/recipes/${recipe.id}`} className="recipe-card">
      <div className={`recipe-cover ${image?"has-image":""}`} style={coverStyle}>
        {!image&&<span className="cover-emoji">{recipe.emoji}</span>}
        <span className="cover-status">{statusLabel[recipe.status]}</span>
      </div>
    ...
    {publicCard&&<button type="button" className={`recipe-like-add ${liked?"selected":""}`} disabled={likeBusy} aria-label={liked?`取消点赞 ${recipe.title}`:`点赞 ${recipe.title}`} title={liked?"已点赞，点击取消":"喜欢这道公开菜谱"} onClick={onToggleLike}><span aria-hidden="true">{liked?"♥":"♡"}</span><b>{likeCount}</b></button>}
