...
export function AppShell({children}:{children:React.ReactNode}) {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><Link className="brand" href="/"><span className="brand-mark">♨</span><span><strong>CookingApp</strong><span>My cooking knowledge base</span></span></Link><nav className="nav" aria-label="主要导航">{nav.map(([href,icon,label])=><Link key={href} href={href} className={active(href)?"active":""} aria-current={active(href)?"page":undefined}><span className="nav-icon">{icon}</span>{t(label)}</Link>)}</nav><div className="sidebar-note"><b>Workflow</b><br/>{t("shell.workflow")}</div></aside>
    ...
    {moreOpen&&<div className="mobile-more-layer"><button className="mobile-more-backdrop" aria-label="关闭更多功能" onClick={()=>setMoreOpen(false)}/><section className="mobile-more-sheet" role="dialog" aria-modal="true" aria-labelledby="mobile-more-title"><div className="mobile-more-head"><div><p className="eyebrow">{t("shell.functions")}</p><h2 id="mobile-more-title">{t("nav.more")}</h2></div><button className="mobile-more-close" aria-label="关闭更多功能" onClick={()=>setMoreOpen(false)}>×</button></div><div className="mobile-account-card"><span className={`avatar ${cloudStatus==="connected"?"is-connected":"is-offline"}`}>越</span><div><small>{t("shell.account")}</small><b>{email||statusLabel}</b></div><Link href="/settings" onClick={()=>setMoreOpen(false)}>{t("shell.openSettings")} →</Link></div><div className="mobile-language-row"><span>{t("language.label")}</span><LanguageSelect compact/></div><nav className="mobile-more-grid" aria-label="移动端更多功能">{mobileMore.map(([href,icon,label])=><Link key={href} href={href} onClick={()=>setMoreOpen(false)} className={active(href)?"active":""} aria-current={active(href)?"page":undefined}><b>{icon}</b><span>{t(label)}</span></Link>)}</nav></section></div>}
    <nav className="mobile-nav" aria-label="移动端导航">{mobile.map(([href,icon,label])=><Link key={href} href={href} className={active(href)?"active":""} aria-current={active(href)?"page":undefined}><b>{icon}</b>{t(label)}</Link>)}<button type="button" className={mobileMore.some(([href])=>active(href))||moreOpen?"active":""} aria-expanded={moreOpen} aria-controls="mobile-more-title" onClick={()=>setMoreOpen(value=>!value)}><b>•••</b>{t("nav.more")}</button></nav>
  </div>;
}
