...
export function AppShell({children}:{children:React.ReactNode}) {
  ...
  useEffect(()=>{if(!authenticated){setEmail("");return;}void connectSupabase().then(s=>s?.auth.getUser()).then(result=>setEmail(result?.data.user?.email||""));},[authenticated]);
  ...
  return <div className="app-shell">
    ...
    <main className="content"><header className="topbar"><Link className="brand" href="/" style={{margin:0}}><span className="brand-mark">♨</span><span><strong>CookingApp</strong></span></Link><span className="topbar-title">{t("shell.tagline")}</span><div className="topbar-actions"><LanguageSelect compact/><span className="status-pill">{statusLabel}</span><Link className="avatar" href="/settings" aria-label={t("shell.openSettings")}>越</Link></div></header>{refreshing&&<div className="sync-banner" role="status"><span/>正在后台同步最新菜谱；页面可以先使用。</div>}{children}</main>
