...
const blank:Recipe={id:"",title:"",summary:"",emoji:"🍳",color:"linear-gradient(135deg,#e8c990,#d68353)",servings:2,totalMinutes:30,difficulty:"简单",riskLevel:"需留意",riskNote:"",effortType:"日常菜",status:"to_try",visibility:"private",tags:[],tools:[],ingredients:[{id:"i-1",name:"",amount:"",unit:""}],steps:[{id:"s-1",instruction:""}],versionNote:"",updatedAt:""};
...
export function RecipeEditor({initial}:{initial?:Recipe}) {
  ...
  const submit=async(e:React.FormEvent)=>{
    ...
    const id=recipe.id||`${Date.now()}-${recipe.title.replace(/\s+/g,"-").slice(0,24)}`;
    try{
      await saveRecipe({...recipe,id,visibility:"private",updatedAt:new Date().toISOString().slice(0,10),tags:recipe.tags.map(tag=>tag.trim()).filter(Boolean)});
      setSaved(true);
      router.push(`/recipes/${id}`);
    }catch(reason){setError(reason instanceof Error?reason.message:"菜谱保存失败，请稍后重试。");}
  ...
  return <form onSubmit={submit} className="two-col">
    ...
    <section className="panel"><h2>基本信息</h2><div className="form-grid">
      ...
      <div className="field"><label>操作风险</label><select value={recipe.riskLevel||"需留意"} onChange={e=>update("riskLevel",e.target.value as Recipe["riskLevel"])}><option>低风险</option><option>需留意</option><option>高风险</option></select><small>风险与难度分开：高温油、明火、压力锅等应单独说明。</small></div>
      <div className="field"><label>耗时类型</label><select value={recipe.effortType||"日常菜"} onChange={e=>update("effortType",e.target.value as Recipe["effortType"])}><option>快手菜</option><option>日常菜</option><option>功夫菜</option></select><small>功夫菜表示等待或照看时间长，不等于操作危险。</small></div>
      <div className="field full"><label>风险提示</label><textarea value={recipe.riskNote||""} onChange={e=>update("riskNote",e.target.value)} placeholder="例如：下锅前擦干食材；油温需要经验判断；糖浆和热油避免接触皮肤"/></div>
    ...
    <aside><div className="panel" style={{position:"sticky",top:96}}><h2>我的版本</h2><div className="field"><label>本次版本说明</label><textarea value={recipe.versionNote} onChange={e=>update("versionNote",e.target.value)} placeholder="例如：糖减半；最后提高温度上色"/></div><div className="notice" style={{marginTop:16}}><b>私人原稿</b><br/>保存只会更新你自己的菜谱。需要进入公开菜谱库时，请保存后在菜谱详情页提交“申请公开”，由管理员审核通过后发布快照。</div><div className="notice" style={{marginTop:12}}>用量、温度、风险、无麸质和替代品如果没有确认，请保留为空或写明“待核验”，不要根据视频标题猜测。</div>{error&&<div className="notice notice-error" role="alert" style={{marginTop:12}}>{error}</div>}<button className="btn btn-primary" style={{width:"100%",marginTop:18}} type="submit" disabled={saving}>{saving?"正在保存…":"保存菜谱"}</button></div></aside>
